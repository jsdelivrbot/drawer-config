...
.run(['taRegisterTool', '$window', 'taTranslations', 'taSelection', function (taRegisterTool, $window, taTranslations, taSelection) {
    ...
    taRegisterTool('fontColor', {
        ...
        options: {
            replacerClassName: 'fa fa-font', showButtons: false, showInput: true, preferredFormat: 'hex'
        },
        color: "#000"
    });
    taRegisterTool('cellColor', {
        display: "<div spectrum-colorpicker ng-model='promo.bannerContent.HTML.backgroundColor' on-change='action(color)' format='\"hex\"' options='options'></div>",
        action: function (cellColor) {
            var me = this;
            if (!this.$editor().wrapSelection) {
                setTimeout(function () {
                    me.action(cellColor);
                ...
                jQuery('.main').css('background-color',cellColor);
            }
        },
        options: {
            showButtons: false
